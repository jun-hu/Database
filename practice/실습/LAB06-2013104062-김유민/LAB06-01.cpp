...
int main()
{
	...
	//1번문제 - delete문제 해결
	//DBPrepare((SQLCHAR*)"delete from \"Jobs_copy\" where job_title = 'Computer Engineer'");
	//DBExecuteSQL((SQLCHAR*)"delete from \"Jobs_copy\" where job_title = 'Computer Engineer'");

	//1번문제 - update문제 해결(max_salary 500늘리기)
	//DBPrepare((SQLCHAR*)"update \"Jobs_copy\" set max_salary = max_salary + 500 where job_title like '%Marketing%'");
