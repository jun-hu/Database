...
{
	SQLINTEGER salary;
	SQLCHAR department[30], country[40];


	SQLLEN i_salary, i_country, i_department;

	//select 질의 수행 결과를 저장할 변수를 지정한다.(Column Number는 1부터 시작한다)

	SQLBindCol(hStmt, 1, SQL_CHAR, department, sizeof(department), &i_department);
	SQLBindCol(hStmt, 2, SQL_CHAR, country, sizeof(country), &i_country);
	SQLBindCol(hStmt, 3, SQL_INTEGER, &salary, sizeof(salary), &i_salary);
	//질의 수행결과를 hStmt로 가져온다. 이때, 바인딩된 변수에 값이 할당된다.
	while (SQLFetch(hStmt) != SQL_NO_DATA)
	{
		cout << department << ' ' << country << ' ' << salary << ' ' << endl;
	}
...
int main()
{
	...
	DBExecuteSQL((SQLCHAR*)"select department_name,country_name, \"salary\" from(select d.department_name, c.country_name, sum(e.salary) as \"salary\" from \"Countries_copy\" c, (select * from \"Employees_copy\" where salary >= 10000) e, \"Locations_copy\" l, \"Departments_copy\" d where e.department_id = d.department_id and d.location_id = l.location_id and l.country_id = c.country_id group by c.country_name, d.department_name order by \"salary\" desc) where rownum < 3");
